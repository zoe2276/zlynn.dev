<template>
    <div id="page-projects">
        <pre>these are my projects</pre>
        <div>
            <ul>
                <Project title="this website" link="https://github.com/zoe2276/zlynn.dev" description="website made in Vue" />
                <Project title="auth.zlynn.dev" link="https://github.com/zoe2276/go-auth" description="authentication backend for zlynn.dev, written in Go" />
                <Project title="monitorman" link="https://github.com/zoe2276/monitorman" description="a monitor dimension manager written in Java. work in progress" />
                <Project title="loopi" link="https://github.com/zoe2276/loopi" description="a stitch counter and project tracker for knitting/crocheting. written in React" />
            </ul>
            
        </div>
        <Console delay="3"/>
    </div>
</template>

<script setup>
import Project from '../components/Project.vue';
import Console from '../components/Console.vue';
</script>

<style>
@keyframes slideUp {
    from { max-height: 100vh }
    to { max-height: 0 }
}

@keyframes slideDown {
  from { max-height: 0 }
  to { max-height: 100vh }
}
</style>

<style scoped>
#page-projects {
    animation: slideDown 1s steps(17, end) forwards;
    height: 100%;
    display: flex;
    flex-flow: column nowrap;
    justify-content: space-between;
}
</style>